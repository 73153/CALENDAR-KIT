...
- (BOOL) substringIsInDictionary:(NSString *)subString
{
    ...
    if (_autoCompleteDelegate && [_autoCompleteDelegate respondsToSelector:@selector(autoCompletion:suggestionsFor:)]) {
        self.suggestionsDictionary = [_autoCompleteDelegate autoCompletion:self suggestionsFor:[subString capitalizedString]];
    }
    
    for (NSString *tmpString in self.suggestionsDictionary)
    {
        range = ([[self.options valueForKey:ACOCaseSensitive] isEqualToNumber:[NSNumber numberWithInt:1]]) ? [tmpString rangeOfString:[subString capitalizedString]] : [tmpString rangeOfString:[subString capitalizedString] options:NSCaseInsensitiveSearch];
