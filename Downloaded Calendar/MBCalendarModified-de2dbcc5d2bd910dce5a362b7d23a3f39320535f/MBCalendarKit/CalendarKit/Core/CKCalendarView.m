...
- (id)init
{
    ...
    if (self) {
        ...
        [event setDate:[NSDate date]]; //this date is cleaned up in CKDEMOVC
...
- (void)reloadAnimated:(BOOL)animated
{
    if ([[self dataSource] respondsToSelector:@selector(calendarView:eventsForDate:)]) {
        ...
        NSDateComponents *components = [[NSCalendar currentCalendar] components:kCFCalendarUnitDay | kCFCalendarUnitMonth | kCFCalendarUnitYear fromDate:[self date]];
        NSInteger day = [components day];
        NSInteger month = [components month];
        NSInteger year = [components year];
        NSArray *sortedArray = [[[self dataSource] calendarView:self eventsForDate:[NSDate dateWithDay:day month:month year:year]] sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
            NSDate *d1 = [obj1 date];
            NSDate *d2 = [obj2 date];
