...
- (void)viewDidLoad
{
   ...
   // [[self ]]
    ...
    UIBarButtonItem *addEvent = [[UIBarButtonItem alloc] initWithTitle:@"Add Event" style:UIBarButtonItemStyleBordered target:self action:@selector(unwindEvents)];
    
    [self setToolbarItems:@[todayButton, item, addEvent] animated:NO];
...
-(void)unwindEvents
{
    self.controller = [[NewCKCalendarEventViewController alloc] initWithNibName:@"CKNewEvent" NSDate:self.calendarView.date bundle:nil];
...
-(void)didDismissSecondViewController {
    NSLog(@"I have been dismissed.");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //[self.events addObject: self.controller.event];
    [[self dataSource] calendarView:self.calendarView addEvent:self.controller.event toDate:self.calendarView.date];
    // [[self events] addObject:self.controller.event];
...
{
    if ([[self dataSource] respondsToSelector:@selector(calendarView:addEvent:toDate:)]) {
        [[self dataSource] calendarView:calendarView addEvent:event toDate:date];
    }
}
